...
public class WithdrawalCreatedHookMessageGenerator extends BaseHookMessageGenerator<Withdrawal> {
    ...
    protected WebhookMessage generateMessage(Withdrawal event, WebHookModel model, MessageGenParams messageGenParams) {
        try {
            ...
            withdrawalStarted.setOccuredAt(OffsetDateTime.parse(messageGenParams.getCreatedAt(), DateTimeFormatter.ISO_DATE_TIME));
            ...
            log.error("Error when generate webhookMessage event: {} model: {} eventId: {} e: ", event, model, messageGenParams.getEventId(), e);
